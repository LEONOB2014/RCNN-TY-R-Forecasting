...
class DeconvGRUCell(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, kernel_size):
        ...
        self.input_size = input_size
        self.hidden_size = hidden_size
        self.reset_gate = nn.ConvTranspose2d(input_size+hidden_size, hidden_size, kernel_size, padding=padding)
        self.update_gate = nn.ConvTranspose2d(input_size+hidden_size, hidden_size, kernel_size, padding=padding)
        self.out_gate = nn.ConvTranspose2d(input_size+hidden_size, hidden_size, kernel_size, padding=padding)
    ...
    def forward(self, input_=None, prev_state=None):
        # get batch and spatial sizes
        batch_size = prev_state.data.size()[0]
        spatial_size = prev_state.data.size()[2:]
